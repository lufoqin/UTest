...
import shutil
import ast
from configs.settings import test_result_tmp_dir
from configs.settings import tmp_result_str_separator as separator
from configs.settings import tmp_result_str_line_separator as l_separator


class ResultSave(WriteHelper):
    def __init__(self):
        super(ResultSave, self).__init__()
        self.sheet_title = ["casename", "url", "method", "params", "header", "response", "status_code", "response_time", "error_info"]
    ...
    def append_row(self, sheet_name, data_list, passed):
        ...
        elif passed.upper() == 'XFAIL':
            style = self.set_style('orange')
        elif passed.upper() == 'XPASS':
            style = self.set_style('cyan_ega')
        else:
            style = self.set_style('green')
        self.append(sheet_name, data_list, style)

    def write_result(self, file_name: str, datalist: list, passed: str):
        """
        为每个测试套保存测试结果到json文件中以供后续写入Excel文件中
        :param file_name:
        :param datalist:
        :param passed:
        :return:
        ...
        if not os.path.isdir(test_result_tmp_dir):
            log.info('test_result_tmp_dir is not existed, create it')
            os.makedirs(test_result_tmp_dir)

        data_str = ""
        for item in datalist:
            data_str += "{}{}".format(item, separator)

        data_str += "{}{}".format(passed.upper(), l_separator)
        fn = os.path.join(test_result_tmp_dir, "{}.data".format(file_name))
        log.info('write test result to {}'.format(fn))
        with open(fn, 'a', encoding='utf-8') as f:
            f.write(data_str)

    def gen_excel_report(self, file_path):
        log.info('start writing result to excel file')
        if not os.path.isdir(test_result_tmp_dir):
            log.warning('test_result_tmp_dir is not create, no need to generate excel report')
            return
        file_list = os.listdir(test_result_tmp_dir)
        log.info('all the file to be processed: {}'.format(file_list))
        for file_name in file_list:
            if not os.path.isfile(os.path.join(test_result_tmp_dir, file_name)):
                continue
            with open(os.path.join(test_result_tmp_dir, file_name), 'r', encoding='utf-8') as f:
                text_content = f.read()
            line_content_list = text_content.split(l_separator)
            for line_content in line_content_list:
                content_item_list = line_content.split(separator)
                passed = content_item_list.pop(-1)
                self.append_row(file_name.rstrip('.data'), content_item_list, passed)
        result_name = self.save(file_path)
        log.info('write result to excel file complete')
        return result_name
...
class AllureReport(object):
    ...
    def create_allure_report(self, report_dir, **kwargs):
        ...
        obj.communicate()
        if isopen.lower() == "open":
            port = kwargs.get('port')
            ip = kwargs.get('ip')
            return self.open_report_server(suffix_dir, ip, port)
    ...
    def kill_process_by_port_on_linux(cls, port):
        ...
        find_cmd = """ps -aux |grep allure |grep -v grep |awk '{print $2}'"""
        kill_cmd = "kill -9 {}"
        with os.popen(find_cmd) as res:
            pid = res.read().strip()
        if pid:
