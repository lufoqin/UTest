...
class Generator(object):
    def __init__(self, task):
        ...
        self.global_vars = task.global_vars
    ...
    def generate_testsuite(self):
        for sheet_name, testsuites in self.testsuites.items():
            ...
            local_vars = testsuites.get('local_vars') or []
            py_code = ""
            py_code += self._create_header_code()  # 读取header_template模板文件
            py_code += self._create_save_interface_vars_code()  # 读取save_interface_vars_template模板文件
            py_code += self._create_setup_class_case_code(local_vars=local_vars)  # 在测试套的setup_class中处理局部变量的内容
            if init_case:
                py_code += self._create_testcase_code(init_case, prefix="test_init",  template="inclusive_template")
            if restore_case:
                py_code += self._create_testcase_code(restore_case, prefix="test_restore", template="inclusive_template")
            if testcase:
                py_code += self._create_testcase_code(testcase, template="testcase_template")
    ...
    def generate_common_test(self):
        ...
        with open(os.path.join(self.testcase_dir, "task_common.py"), 'w', encoding='utf-8') as f:
            f.write(py_code)
    ...
    def _create_save_interface_vars_code(self):
        ...
        return Template(code_str).substitute()

    def _create_testcase_code(self, testsuite, **kwargs):
        """
        创建测试sheet中的用例和测试sheet中的初始化和恢复用例
        :param testsuite:
        :param kwargs:
        :return:
        """
        ...
        template_name = kwargs.get('template') or "testcase_template"
        code_str = self._read_file(template_name)
        replaced = ""
        default_index = 0
        existing = []  # 用来规避测试方法重名

        for tesecase in testsuite:
            order = tesecase.get('order')
            module = tesecase.get('module') or "no set module info"
            params = tesecase.get('params')
            casename = "{}_{}".format(prefix, tesecase.get('casename')) if tesecase.get('casename') else \
                "{}_autotest_{}".format(prefix, default_index)
            if casename in existing:
                casename = "{}_1".format(casename)
            existing.append(casename)
            description = tesecase.get('description') or "no description"
            init_case = tesecase.get('init') or []
            restore_case = tesecase.get('restore') or []
    ...
    def _create_setup_class_case_code(self, **kwargs):
        local_vars = kwargs.get('local_vars') or []
        code_str = self._read_file("setup_class_template")
        return Template(code_str).substitute(local_vars=local_vars)
    ...
    def _create_common_header(self):
        return Template(self._read_file("task_common_header_template")).substitute(global_vars=self.global_vars)
