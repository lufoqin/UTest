...
from core.case_parse.toolset import PreProcess,generate_random_str
...
class Process(object):
    ...
    def __init__(self, filepath):
        ...
        self.testsuites: dict = {}  # 除了已有固定的sheet外的其他测试sheet
        ...
        self.testsuites = self.check_need_test_task()
        ...
        for row in list(rows)[1:]:
            ...
            if key_content:
                ...
                if key_content == 'hostname':
                    ...
                    if mail_switch.lower() == 'true' or mail_switch.lower() == "on":
                        mail_switch = True
                    else:
                        mail_switch = False
                    ...
                    if dingrobot_switch.lower() == "true" or dingrobot_switch.lower() == "on":
                ...
                elif key_content == 'need_to_test' or key_content == '需要测试' or key_content == '需要测试的':
                    need_to_test = value_content or None
                    if need_to_test:
                        need_to_test = [item.strip() for item in need_to_test.split(',')]
                    base_configs_dict['need_to_test'] = need_to_test
                elif key_content == 'no_testing_required' or key_content == '不需要测试' or key_content == '不需要测试的':
                    no_test = value_content or None
                    if no_test:
                        no_test = [item.strip() for item in no_test.split(',')]
                    base_configs_dict['no_test'] = no_test
    ...
    def check_need_test_task(self):
        """
        func: 根据基础配置文件中配置的内容，进行识别哪些用例需要进行测试
        :return:
        """
        need_test_list = self.base_configs.get('need_to_test') or None
        no_test_list = self.base_configs.get('no_test') or None
        testsuites = self.testsuites
        updated_testsuites = {}
        if need_test_list is None or 'all' in need_test_list or 'ALL' in need_test_list or 'All' in need_test_list:
            log.info('need to test sheet is: all')
        else:
            for name, item in testsuites.items():
                if name in need_test_list:
                    log.info('need to test: {}'.format(name))
                    updated_testsuites[name] = item
            return updated_testsuites

        if no_test_list is None:
            log.info('no testing required is: {}'.format('None'))
        elif no_test_list == ['all'] or no_test_list == ['All'] or no_test_list == ['ALL']:
            return {}
        else:
            updated_testsuites = testsuites
            for name in list(testsuites.keys()):
                if name in no_test_list:
                    log.info('no need to test: {}'.format(name))
                    del updated_testsuites[name]
        ...
        for sheet in sheets:
            ...
            for case_type, lines in case_line_info.items():
                if case_type == self.testsuite_init_line_name:
                    ...
                    log.info('testsuite local vars content is:{}'.format(testsuites[sheet_name]["local_vars"]))
    ...
    def serialize_testsuite(self, testsuite):
        ...
        for case_type, testcase in testsuite.items():
            case_serialize = []
            if case_type == 'local_vars':
                testsuite_serialize[case_type] = testcase
            else:
    ...
    def add_random_str_for_localvar(self, localvarrow):
        """
        :auth: lfy
        该方法暂未使用，为了方便后续维护，对应的功能统一调整到toolset的PreProcess类中处理
        func: 遍历局部变量，如果需要添加随机8位随机字符即添加
        :return:
        """
        localvarrow=localvarrow
        localvarrow=json.dumps(localvarrow) 
        pattern = re.compile(r"\$\S+\$")
        key_words = re.findall(pattern, localvarrow)
        if key_words:
            for key_word in key_words:
                real_word = key_word.lstrip('$').rstrip('$')
                localvarrow = localvarrow.replace(key_word, real_word + generate_random_str(8))
        localvarrow = json.loads(localvarrow)
        return localvarrow
    
    def add_random_num_for_case(self, case={}):
        """
        auth: lfy
        该方法暂未使用，为了方便后续维护，对应的功能统一调整到toolset的PreProcess类中处理
        func: 遍历用例参数，替换/d成随机数字0~9
        :return:
        """
        case = case
        for key, value in case.items():
            if "<#p>" in str(value):
                ...
                print(len(re.findall(pattern, value)))
                for i in range(0, len(re.findall(pattern, value))):
                    a = str(random.randint(0, 9))
                    print("生成的数字"+a)
                    value = re.sub(pattern, a, value, 1)
                    print("替换后的字符："+value)
            case[key] = value
        return case
    ...
    def cases_order(self):
        ...
        testsuites = self.testsuites
        base_num = settings.max_num_of_testsheet
        max_init_num = settings.max_num_of_testsheet_init_or_restore
        ...
        for sheet_name, testcases in testsuites.items():
            if testcases:
                ...
                testsheet_init_case = testcases.get('init_case')
                testcase_testcase = testcases.get('testcase')
                testsheet_restore_case = testcases.get('restore_case')

                init_start = case_base_num  # 测试sheet初始化用例开始编排的order号
                testcase_start_num = case_base_num + max_init_num
                restore_start = case_base_num + max_init_num * 9
                if testsheet_init_case:
                    testcases['init_case'] = self._order_for_testcase(testsheet_init_case, init_start)
                if testsheet_restore_case:
                    testcases['restore_case'] = self._order_for_testcase(testsheet_restore_case, restore_start)
                if testcase_testcase:
                    testcases['testcase'] = self._order_for_testcase(testcase_testcase, testcase_start_num)
...
class Serialize(object):
    ...
    def serialize(self):
        ...
        self.iteration_value_check()

        return self.api_info

    @classmethod
    def _json_serialize(cls, s: str):
        try:
            # s = json.dumps(s)
            return ast.literal_eval(s)
        except Exception as e:
            log.error(string.Template("进行json格式化出错, 出错的内容: $s, 错误信息: $e").substitute(s=s, e=e))
    ...
    def res_header_serialize(self):
        res_header = self.api_info['res_header'].strip().replace('\n', '') if self.api_info.get('res_header') else None
        log.info(string.Template("api request res_header info is: $res_header").substitute(res_header=res_header))
        if not res_header:
            return
        res = self._json_serialize(res_header)
        if res:
            self.api_info['res_header'] = res
            return res
        else:
            log.error(string.Template("api request res_header format error, res_header info: $res_header").substitute(
                res_header=self.api_info['res_header']))
            return False
    ...
    def iteration_value_check(self):
        """
        func: 检查迭代次数的合法性，如果设置的值不能转换为整型的话，则默认设置为1
        :return:
        """
        iteration = self.api_info.get('iteration')
        log.info("api request iteration value is: {}".format(iteration))
        if iteration is None:
            self.api_info['iteration'] = 1
            return 1
        try:
            res = int(iteration)
        except Exception as e:
            log.error('integer iteration fail, set the iteration to 1, the error info is: {}'.format(e))
            res = 1

        self.api_info['iteration'] = res
        return res
