...
log_devel = "DEBUG"
log_max_size = 50 * 1024 * 1024  # 单个log文件最大文件大小
log_backup_count = 5  # 保留log文件的个数

# 模板用例存放目录
template_file_path = os.path.join(current_path, "static", "case_template")
...
# 整个工程的临时目录
project_tmp_dir = os.path.join(current_path, ".tmp")

# 存放工程由初始化sheet和全局变量sheet转换的全局变量，以供使用多并发执行时每个进程获取对应的全局变量
save_global_var_tmp_file = os.path.join(project_tmp_dir, "global_vars.json")

# 存放每个测试套产生的测试结果的临时存放路径
test_result_tmp_dir = os.path.join(project_tmp_dir, "test_result")
# 测试结果摘要临时存放文件
test_result_summary_file = os.path.join(project_tmp_dir, "report_summary.data")
# 临时保存结果的数据分隔符
tmp_result_str_separator = "<#separator#>"
# 临时保存结果的行分隔符
tmp_result_str_line_separator = "<#line-separator#>"
...
# 测试用例执行顺序相关的配置
max_num_of_testsheet = 10000  # 单个测试sheet最大的用例个数
max_num_of_testsheet_init_or_restore = 1000  # 单个测试sheet最大的初始化用例个数
